...
def end_video_viewing_1(participant):
    """End first round of video viewing and redirect based on group."""
    # Control group (0) skips message page
    if participant.group_number == 0:
        return redirect(url_for('round2.select_categories_round2'))
    # Treatment groups see message
...
def additional_information(participant):
    """Handle additional information form processing. Control group (0) skips this page."""
    group_num = participant.group_number
    
    # Control group (group 0) skips message page entirely
    if group_num == 0:
        return redirect(url_for('round2.select_categories_round2'))
    ...
    if request.method == 'POST':
        try:
            ...
            # All groups now go to second category selection
            return redirect(url_for('round2.select_categories_round2'))
